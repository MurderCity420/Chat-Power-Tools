(function () {
    ...
    const DEFAULTS = {
        ...
        hideAllTips: false,
        autoRateBack: false,
        autoRate5Target: 'friends_favs',    // 'all' | 'friends_favs' | 'friends_only' | 'favs_only'
        autoRate4Back: false,
        ...
        autoBoom: false,
        autoBoomTarget: 'friends_favs',     // 'all' | 'friends_favs' | 'friends_only' | 'favs_only'
        ...
        autoBoomCustom: [],
        // Auto-reply: fuzzy-match incoming chat messages and auto-send a reply.
        // Never fires on your own messages. Each rule: { match, reply }.
        autoReply: false,
        autoReplyTarget: 'friends_favs',    // 'all' | 'friends_favs' | 'friends_only' | 'favs_only'
        autoReplyRules: [{ match: 'cheers', reply: 'cheers' }],
        // Default room-user-list gender filter, applied once at login.
        // true = show that gender, false = hide it by default.
        defaultGenderFilter: { female: true, male: true, tranny: true, couple: true, group: true },
        // Hourly time-zone shout-out: at :20 past every UTC hour, post a message
        // plus the cities currently at local midnight. Fixed offsets (no DST).
        hourlyTzPost: false,
        hourlyTzMessage: '',
        // Auto-cam: open the cam of anyone who appears in your My Viewers list.
        autoCam: false,
        autoCamTarget: 'friends_favs',      // 'all' | 'friends_favs' | 'friends_only' | 'favs_only'
        autoCamMax: 4,             // 1-15 — max simultaneous open cams before it stops
        // Cam templates: saved floating-cam layouts (up to 12 positions each).
        camTemplateEnabled: false,
        camTemplateSelectedId: '',   // live/active layout (top-nav dropdown)
        camTemplateDefaultId: '',    // layout applied at login (Automations tab)
        camTemplates: [],            // [{ id, desc, slots: [{top,left,width,height}, ...] }]
        ...
        tabVisibility: {
            ...
            features: true, automations: true, advanced: true, blockedyou: true, snapshots: true,
            power: false, test: false, data: true,
        },
        revealBlockedYou: false,
        // Fan mail lives on the Automations tab now. Off by default; when on it
        // adds a template dropdown to the site's Fan Mail compose page.
        fanMailEnabled: false,
        fanMailTemplates: [],
