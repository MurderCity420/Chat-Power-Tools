    ...
    function renderFanMailPane() {
        const pp = document.querySelector('#pt-panel .pt-tabpane[data-pane="fanmail"]');
        if (!pp) return;
        const templates = settings.fanMailTemplates || [];
        const tokenOptionsHtml = FM_TOKEN_OPTIONS.map(v => `<option value="${v}">${v}</option>`).join('');

        pp.innerHTML = `
            <div class="pt-section">
                <h3>Fan Mail templates <span class="pt-info" data-tip="Saved Fan Mail templates. Use the dropdown that appears on the site's Fan Mail compose page, or send via these arrow buttons after opening a profile.">i</span></h3>
                <div style="display:flex;gap:6px;margin-bottom:8px">
                    <button id="pt-fm-new" class="pt-btn-primary">+ New</button>
                    <button id="pt-fm-random" class="pt-btn-primary">Random Subjects</button>
                </div>
        ...
        const list = pp.querySelector('#pt-fm-list');
        if (templates.length === 0) {
            list.innerHTML = '<div style="color:#666;font-style:italic;padding:8px">No templates yet — click + New to create one.</div>';
        } else {
            ...
            templates.forEach((t, idx) => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="padding:4px 6px;text-align:right;white-space:nowrap">
                        <button class="pt-fm-send" data-idx="${idx}" title="Send this template (compose page must be open)">${sendIcon}</button>
        ...
        // Wire up buttons
        pp.querySelector('#pt-fm-new').addEventListener('click', () => openFanMailEditor(null));
        pp.querySelector('#pt-fm-random').addEventListener('click', () => openFanMailRandomEditor());

        pp.querySelectorAll('.pt-fm-send').forEach(btn => {
            btn.addEventListener('click', () => {
                const idx = parseInt(btn.dataset.idx, 10);
                applyFanMailTemplateNow(templates[idx]);
            });
        });
        pp.querySelectorAll('.pt-fm-up').forEach(btn => {
            btn.addEventListener('click', () => {
                const idx = parseInt(btn.dataset.idx, 10);
                if (idx > 0) {
        ...
        pp.querySelectorAll('.pt-fm-down').forEach(btn => {
            btn.addEventListener('click', () => {
                const idx = parseInt(btn.dataset.idx, 10);
        ...
        pp.querySelectorAll('.pt-fm-del').forEach(btn => {
            btn.addEventListener('click', () => {
                const idx = parseInt(btn.dataset.idx, 10);
    ...
    // Triggered from the Fan Mail tab list's ▶ button.
    // Requires the user to already have the Fan Mail compose page open in the
    // profile popup iframe (otherwise we don't know who to send to).
    function applyFanMailTemplateNow(template) {
        const doc = getProfileIframeDoc();
        if (!doc) {
            alert('Open a user profile and click "Fan Mail" first, then run this from there.');
            return;
        }
        const onMailPage = !!doc.querySelector('input[name="subject"]') && !!doc.querySelector('textarea[name="message"]');
        if (!onMailPage) {
            alert('Open the Fan Mail compose page first (click Fan Mail on a user profile), then try again.');
            return;
        }
        const ok = applyFanMailTemplate(template, doc);
        if (ok) {
            // Also update the dropdown to reflect the chosen template
            const dd = doc.getElementById('pt-fm-iframe-dd');
            if (dd) {
                const templates = settings.fanMailTemplates || [];
                const idx = templates.indexOf(template);
                if (idx > -1) dd.value = String(idx);
            }
        } else {
            alert('Could not fill the Fan Mail form. Make sure the compose page is loaded.');
        }
    }
    ...
    function installFanMailIframeWatcher() {
        const checkIframe = () => {
            ...
            try {
                ...
                // Already injected?
                if (doc.getElementById('pt-fm-iframe-dd')) return;
