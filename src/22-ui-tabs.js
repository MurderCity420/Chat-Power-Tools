    ...
    function renderPanelLists() {
        ...
        fp.innerHTML = `
            ...
            <div class="pt-section">
                ...
                <ul class="pt-list" id="pt-fav-list" style="height:240px;max-height:240px"></ul>
            </div>
        `;
    ...
    function renderFeaturesPane() {
        ...
        fp.innerHTML = `
            ...
            <div class="pt-section">
                ...
                <div style="margin-top:8px">
                    <div style="color:#ccc;margin-bottom:4px">User list filter ${_infoLink('features.md', 'user-list-filter')}</div>
                    <div style="color:#aaa;font-size:11px;margin-bottom:4px">Default gender filter (applied at login):</div>
                    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
                        <label style="color:#ccc;font-size:12px"><input type="checkbox" class="pt-genderfilter" data-g="female"> Female</label>
                        <label style="color:#ccc;font-size:12px"><input type="checkbox" class="pt-genderfilter" data-g="male"> Male</label>
                        <label style="color:#ccc;font-size:12px"><input type="checkbox" class="pt-genderfilter" data-g="tranny"> Trans</label>
                        <label style="color:#ccc;font-size:12px"><input type="checkbox" class="pt-genderfilter" data-g="couple"> Couple</label>
                        <label style="color:#ccc;font-size:12px"><input type="checkbox" class="pt-genderfilter" data-g="group"> Group</label>
                    </div>
                    <div style="color:#888;font-size:11px;margin-top:2px">Unchecked genders are hidden in the room user list at login.</div>
                </div>
        ...
        // Default gender filter checkboxes (checked = show that gender at login).
        const gf = settings.defaultGenderFilter || {};
        fp.querySelectorAll('.pt-genderfilter').forEach((cb) => {
            cb.checked = gf[cb.dataset.g] !== false;
            cb.addEventListener('change', () => {
                const cur = Object.assign({ female: true, male: true, tranny: true, couple: true, group: true }, settings.defaultGenderFilter || {});
                cur[cb.dataset.g] = cb.checked;
                saveSetting('defaultGenderFilter', cur);
            });
        });
    ...
    function renderTestPane() {
        ...
        tp.innerHTML = `
            ...
            <div class="pt-section">
                <h3>Hourly time-zone shout-out <span class="pt-info" data-tip="At :20 past every hour (UTC) this posts your message followed by the cities currently at local midnight — the part of the world starting a new day. The zone rotates each hour. Offsets are fixed (no daylight-saving adjustment).">i</span></h3>
                <div class="pt-toggle"><input type="checkbox" id="pt-tzpost-enable"><label for="pt-tzpost-enable">Post a message at :20 past every hour (UTC)</label></div>
                <textarea id="pt-tzpost-msg" placeholder="Your message — the cities currently at midnight are appended automatically" style="width:100%;box-sizing:border-box;height:60px;margin-top:4px;background:#111;color:#eee;border:1px solid #444;border-radius:3px;padding:6px;font-size:12px;resize:vertical"></textarea>
                <div class="pt-row" style="margin-top:6px">
                    <button id="pt-tzpost-preview">Post now (test)</button>
                </div>
                <div id="pt-tzpost-hint" style="color:#888;font-size:11px;margin-top:4px"></div>
            </div>
        `;
        ...
        // Hourly time-zone shout-out bindings
        const tzEnable = tp.querySelector('#pt-tzpost-enable');
        const tzMsg    = tp.querySelector('#pt-tzpost-msg');
        const tzHint   = tp.querySelector('#pt-tzpost-hint');
        const updateTzHint = () => {
            try {
                const cities = (typeof _tzMidnightCities === 'function') ? _tzMidnightCities() : [];
                tzHint.textContent = cities.length
                    ? 'Right now it would append: ' + cities.join(', ')
                    : 'Right now no listed zone is at midnight.';
            } catch (e) { tzHint.textContent = ''; }
        };
        tzEnable.checked = !!settings.hourlyTzPost;
        tzMsg.value = settings.hourlyTzMessage || '';
        updateTzHint();
        tzEnable.addEventListener('change', () => saveSetting('hourlyTzPost', tzEnable.checked));
        tzMsg.addEventListener('input', () => saveSetting('hourlyTzMessage', tzMsg.value));
        tp.querySelector('#pt-tzpost-preview').addEventListener('click', () => {
            updateTzHint();
            const ok = (typeof _tzPostNow === 'function') ? _tzPostNow() : false;
            const btn = tp.querySelector('#pt-tzpost-preview');
            const orig = btn.textContent;
            btn.textContent = ok ? '✓ Posted' : '✗ Not sent (type a message / log in)';
    ...
    const AR_MAX_TEXT = 100;   // auto-reply match/reply length cap
    const AR_MAX_RULES = 25;
    ...
    function renderAutomationsPane() {
        ...
        ap.innerHTML = `
            ...
            <div class="pt-section">
                <h3>Auto reply</h3>
                <div class="pt-toggle"><input type="checkbox" id="pt-autoreply"><label for="pt-autoreply">Auto-reply to chat messages that match your rules</label></div>
                <div id="pt-autoreply-sub" style="padding-left:24px;border-left:2px solid #333;margin:2px 0 6px">
                    <div style="margin-bottom:6px">
                        <div style="color:#aaa;font-size:11px;margin-bottom:4px">Reply to messages from:</div>
                        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-reply-target" value="all"> All Users</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-reply-target" value="friends_favs"> Friends &amp; Favorites</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-reply-target" value="friends_only"> Friends Only</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-reply-target" value="favs_only"> Favorites Only</label>
                        </div>
                    </div>
                    <div class="pt-row" style="margin-top:6px">
                        <button id="pt-autoreply-config">Configure replies…</button>
                    </div>
                    <div style="color:#888;font-size:11px;margin-top:4px">
                        Matching is fuzzy — "cheers", "cheeers" and "cheeeers" all match the same rule. Never replies to your own messages. A short random delay is added before sending.
                    </div>
                </div>
            </div>
            <div class="pt-section">
                <h3>Auto cam</h3>
                <div class="pt-toggle"><input type="checkbox" id="pt-autocam"><label for="pt-autocam">Automatically open the cam of anyone who watches you</label></div>
                <div id="pt-autocam-sub" style="padding-left:24px;border-left:2px solid #333;margin:2px 0 6px">
                    <div style="margin-bottom:6px">
                        <div style="color:#aaa;font-size:11px;margin-bottom:4px">Auto-cam viewers from:</div>
                        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-autocam-target" value="all"> All Users</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-autocam-target" value="friends_favs"> Friends &amp; Favorites</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-autocam-target" value="friends_only"> Friends Only</label>
                            <label style="color:#ccc;font-size:12px"><input type="radio" name="pt-autocam-target" value="favs_only"> Favorites Only</label>
                        </div>
                    </div>
                    <div class="pt-row" style="align-items:center;gap:8px;margin-top:6px">
                        <label for="pt-autocam-max" style="color:#ccc;white-space:nowrap">Max cams to open</label>
                        <select id="pt-autocam-max" style="flex:1;background:#111;color:#eee;border:1px solid #444;padding:4px;border-radius:3px">
                            ${Array.from({ length: 15 }, (_, i) => `<option value="${i + 1}">${i + 1}</option>`).join('')}
                        </select>
                    </div>
                    <div style="color:#888;font-size:11px;margin-top:4px">
                        Fills the 4 docked Cam Panel slots first, then floating cams. Stops once this many cams are open (cams you opened yourself count too). Blocked, ignored, and already-open users are skipped.
                    </div>
                </div>
                <div class="pt-toggle" style="margin-top:8px"><input type="checkbox" id="pt-camtpl-enable"><label for="pt-camtpl-enable">Automatically position and size floating cams</label></div>
                <div id="pt-camtpl-sub" style="padding-left:24px;border-left:2px solid #333;margin:2px 0 6px">
                    <div class="pt-row" style="align-items:center;gap:8px">
                        <button id="pt-camtpl-templates" style="white-space:nowrap">Templates</button>
                        <span style="color:#ccc;white-space:nowrap">Default:</span>
                        <select id="pt-camtpl-default" style="flex:1;background:#111;color:#eee;border:1px solid #444;padding:4px;border-radius:3px"></select>
                    </div>
                    <div style="color:#888;font-size:11px;margin-top:4px">
                        The <b>Default</b> layout is applied when you log in. Switch the live layout any time from the dropdown next to the shield icon — that won't change this default.
                    </div>
                </div>
            </div>
            <div class="pt-section">
                <h3>Fan mail</h3>
                <div class="pt-toggle"><input type="checkbox" id="pt-fanmail-enable"><label for="pt-fanmail-enable">Show Fan Mail template picker on the compose page</label></div>
                <div id="pt-fanmail-sub" style="padding-left:24px;border-left:2px solid #333;margin:2px 0 6px">
                    <div class="pt-row" style="margin-top:6px;gap:6px">
                        <button id="pt-fanmail-templates">Templates…</button>
                        <button id="pt-fanmail-subjects">Subjects…</button>
                    </div>
                    <div style="color:#888;font-size:11px;margin-top:4px">
                        Adds a template dropdown to the site's Fan Mail compose page (mail_send.php). Manage your saved templates and the random subject pool here.
                    </div>
                </div>
            </div>
        `;
        ...
        // ---- Auto reply wiring ----
        const arEnable = ap.querySelector('#pt-autoreply');
        const arSubR   = ap.querySelector('#pt-autoreply-sub');
        const syncArSubRVisible = () => { arSubR.style.display = arEnable.checked ? '' : 'none'; };
        arEnable.checked = !!settings.autoReply;
        const curReplyTarget = settings.autoReplyTarget || 'all';
        const replyTargetRadio = ap.querySelector(`input[name="pt-reply-target"][value="${curReplyTarget}"]`);
        if (replyTargetRadio) replyTargetRadio.checked = true;
        syncArSubRVisible();
        arEnable.addEventListener('change', () => {
            saveSetting('autoReply', arEnable.checked);
            syncArSubRVisible();
        });
        ap.querySelectorAll('input[name="pt-reply-target"]').forEach((r) => {
            r.addEventListener('change', () => { if (r.checked) saveSetting('autoReplyTarget', r.value); });
        });
        ap.querySelector('#pt-autoreply-config').addEventListener('click', () => openAutoReplyEditor());

        // ---- Auto cam wiring ----
        const acEnable = ap.querySelector('#pt-autocam');
        const acSub    = ap.querySelector('#pt-autocam-sub');
        const syncAcSubVisible = () => { acSub.style.display = acEnable.checked ? '' : 'none'; };
        acEnable.checked = !!settings.autoCam;
        const curCamTarget = settings.autoCamTarget || 'all';
        const camTargetRadio = ap.querySelector(`input[name="pt-autocam-target"][value="${curCamTarget}"]`);
        if (camTargetRadio) camTargetRadio.checked = true;
        const acMax = ap.querySelector('#pt-autocam-max');
        acMax.value = String(Math.max(1, Math.min(15, parseInt(settings.autoCamMax, 10) || 4)));
        syncAcSubVisible();
        acEnable.addEventListener('change', () => {
            saveSetting('autoCam', acEnable.checked);
            syncAcSubVisible();
            if (acEnable.checked && typeof scanForAutoCam === 'function') { try { scanForAutoCam(); } catch (e) {} }
        });
        ap.querySelectorAll('input[name="pt-autocam-target"]').forEach((r) => {
            r.addEventListener('change', () => { if (r.checked) saveSetting('autoCamTarget', r.value); });
        });
        acMax.addEventListener('change', () => saveSetting('autoCamMax', Math.max(1, Math.min(15, parseInt(acMax.value, 10) || 4))));

        // ---- Cam templates wiring ----
        const ctEnable = ap.querySelector('#pt-camtpl-enable');
        const ctSub    = ap.querySelector('#pt-camtpl-sub');
        const syncCtSubVisible = () => { ctSub.style.display = ctEnable.checked ? '' : 'none'; };
        ctEnable.checked = !!settings.camTemplateEnabled;
        syncCtSubVisible();
        ctEnable.addEventListener('change', () => {
            saveSetting('camTemplateEnabled', ctEnable.checked);
            syncCtSubVisible();
            try { _ctRenderNavDropdown(); } catch (e) {}
            if (ctEnable.checked) { try { _ctRepackAll(); } catch (e) {} }
        });
        // "Default" = the layout applied at login. It does NOT change the live
        // active template (that's the top-nav dropdown) — only the saved default.
        const ctDefault = ap.querySelector('#pt-camtpl-default');
        const fillCtDefault = () => {
            const list = settings.camTemplates || [];
            ctDefault.innerHTML = '<option value="">(None)</option>' +
                list.map((t) => '<option value="' + t.id + '">' + escapeHtml(t.desc || '(unnamed)') + '</option>').join('');
            ctDefault.value = settings.camTemplateDefaultId || '';
        };
        fillCtDefault();
        ctDefault.addEventListener('change', () => saveSetting('camTemplateDefaultId', ctDefault.value));
        ap.querySelector('#pt-camtpl-templates').addEventListener('click', () => openCamTemplateManager());

        // ---- Fan mail wiring ----
        const fmEnable = ap.querySelector('#pt-fanmail-enable');
        const fmSub    = ap.querySelector('#pt-fanmail-sub');
        const syncFmSubVisible = () => { fmSub.style.display = fmEnable.checked ? '' : 'none'; };
        fmEnable.checked = !!settings.fanMailEnabled;
        syncFmSubVisible();
        fmEnable.addEventListener('change', () => {
            saveSetting('fanMailEnabled', fmEnable.checked);
            syncFmSubVisible();
        });
        ap.querySelector('#pt-fanmail-templates').addEventListener('click', () => openFanMailTemplatesModal());
        ap.querySelector('#pt-fanmail-subjects').addEventListener('click', () => openFanMailRandomEditor());
    }
    ...
    // Modal to configure auto-reply rules. Two columns (match | reply), an
    // add row at the top and the existing-rules list below with Remove buttons.
    // Same size/overlay as the auto-boom editor. Live-persists on every change.
    function openAutoReplyEditor() {
        let rules = (settings.autoReplyRules || []).slice();
        ...
        modal.innerHTML = `
            <div class="pt-modal" style="width:480px">
                ...
                <div class="pt-modal-body">
                    <div style="color:#aaa;font-size:11px;margin-bottom:10px">
                        When an incoming message matches the <b>Match</b> text, your <b>Reply</b> is sent to main chat. Matching is fuzzy — repeated letters and minor typos still match (e.g. "cheers" / "cheeers"). Max ${AR_MAX_RULES} rules.
                    </div>
                    <div style="display:flex;gap:6px;margin-bottom:4px;font-size:11px;color:#8af;text-transform:uppercase;letter-spacing:.05em">
                        <span style="flex:1">Match</span>
                        <span style="flex:1">Reply</span>
                        <span style="width:62px"></span>
                    </div>
                    <div style="display:flex;gap:6px;margin-bottom:10px">
                        <input type="text" id="pt-ar-match" maxlength="${AR_MAX_TEXT}" placeholder="word/phrase to match" style="flex:1;box-sizing:border-box">
                        <input type="text" id="pt-ar-reply" maxlength="${AR_MAX_TEXT}" placeholder="reply to send" style="flex:1;box-sizing:border-box">
                        <button id="pt-ar-add" class="pt-btn-primary" style="width:62px">Add</button>
                    </div>
                    <ul id="pt-ar-list" class="pt-list" style="max-height:220px"></ul>
                </div>
                <div class="pt-modal-footer">
                    <button id="pt-ar-done" class="pt-btn-primary">Done</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);

        const list = modal.querySelector('#pt-ar-list');
        const renderRules = () => {
            list.innerHTML = '';
            if (!rules.length) {
                list.innerHTML = '<li class="pt-empty">No reply rules yet.</li>';
                return;
            }
            rules.forEach((r, idx) => {
                const li = document.createElement('li');
                li.innerHTML = `<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis"><strong>${escapeHtml(r.match)}</strong> → ${escapeHtml(r.reply)}</span><button>Remove</button>`;
                li.querySelector('button').addEventListener('click', () => {
                    rules.splice(idx, 1);
                    saveSetting('autoReplyRules', rules);
                    renderRules();
                });
                list.appendChild(li);
            });
        ...
        const matchInput = modal.querySelector('#pt-ar-match');
        const replyInput = modal.querySelector('#pt-ar-reply');
        const addRule = () => {
            const match = matchInput.value.trim().slice(0, AR_MAX_TEXT);
            const reply = replyInput.value.trim().slice(0, AR_MAX_TEXT);
            if (!match) return alert('Enter the text to match.');
            if (!reply) return alert('Enter the reply to send.');
            if (rules.length >= AR_MAX_RULES) return alert('Maximum of ' + AR_MAX_RULES + ' reply rules.');
            if (rules.some((r) => r.match.toLowerCase() === match.toLowerCase())) return alert('You already have a rule matching "' + match + '".');
            rules.push({ match: match, reply: reply });
            saveSetting('autoReplyRules', rules);
            matchInput.value = ''; replyInput.value = '';
            renderRules();
            matchInput.focus();
        };
        modal.querySelector('#pt-ar-add').addEventListener('click', addRule);
        replyInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') addRule(); });
        matchInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') replyInput.focus(); });

        const closeModal = () => modal.remove();
        modal.querySelector('.pt-modal-close').addEventListener('click', closeModal);
        modal.querySelector('#pt-ar-done').addEventListener('click', closeModal);
        modal.addEventListener('click', (e) => { if (e.target === modal) closeModal(); });
    }
