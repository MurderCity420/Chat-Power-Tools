...
- **Auto-translate chat** — detects messages written in another language and replaces them inline with a translation into your language, with a **translate icon** you click to flip back to the original (it also names the original language; messages already in your language are left alone). A **Mode** setting chooses **Automatic** (translate foreign messages as they arrive) or **On click** (only foreign messages get a translate button — click it to reveal the translation, click again to go back). Pick your **target language** (a searchable list with regional variants like Portuguese (Brazil)/(Portugal), or Auto from your browser) and the **engine**: **Google** (default, no setup) or **DeepL** (uses DeepL's own free web endpoint, no key/signup — unofficial and rate-limited, best for lighter use). It works in the **main chat and in IMs** — your own IM and other people's, both the built-in windows and the pop-out IMs. It's per-reader — nothing you send is changed. Translations are cached and rate-limited, so a busy room stays smooth; if the free engine ever throttles, messages simply stay in their original language for a bit (see the **Log** tab, `Translate` module). Works fine alongside a hover-tooltip translator extension like *MouseTooltipTranslator* — this one does the inline replacement. Translated messages carry a small **translate icon (文A)** to flip back to the original.
- **IM timestamps** — the site sends no time with IM messages, so this stamps each IM message with **your local time** as it arrives. Pick how it shows from the dropdown: **Off**, **Inline** (`name: (11:50PM) message`, in your device's time format), **Hover** (the time appears as a tooltip when you hover the message line), or **Both**. Works in the site's IM windows, your own IM, and the **pop-out IMs**. Messages already on screen when you open an IM (history) are left un-timed, since their real time isn't known.
...
- **Max Resolution** — overrides your webcam's capture request with a chosen resolution (**320×240** up to **3840×2160**). Takes effect on your next broadcast start — no reload needed. The **Detect** button (or just starting your cam normally) reads what your actual hardware supports and annotates the dropdown accordingly — Chrome/Edge only, this isn't reported by Firefox/Safari.
- **Clean Audio** — independent of Max Resolution — disables the browser's voice-processing DSP (echo cancellation / noise suppression / auto gain) so music or line-in audio comes through clean instead of being treated like a phone call.
- **Video Stats on Hover** — hover any cam (docked or popped out) to see its live resolution and frame rate in the corner. Works for your own cam and anyone else's — handy for checking whether Max Resolution above is actually taking effect, or just for eyeballing stream quality generally.
