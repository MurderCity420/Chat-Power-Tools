...
- **Force Resolution + Clean Audio** — overrides your webcam's capture request with a chosen resolution (**320×240** up to **3840×2160**) and disables the browser's voice-processing DSP (echo cancellation / noise suppression / auto gain) so music or line-in audio comes through clean instead of being treated like a phone call. Takes effect on your next broadcast start — no reload needed. A **Detect My Camera** button (or just starting your cam normally) reads what your actual hardware supports and annotates the dropdown accordingly — Chrome/Edge only, this isn't reported by Firefox/Safari.
- **Video Stats on Hover** — hover any cam (docked or popped out) to see its live resolution and frame rate in the corner. Works for your own cam and anyone else's — handy for checking whether Force Resolution above is actually taking effect, or just for eyeballing stream quality generally.
